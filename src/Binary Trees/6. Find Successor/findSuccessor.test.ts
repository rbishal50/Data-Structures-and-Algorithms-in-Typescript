...
describe("findSuccessor", () => {
  it("should return the correct successor for a given node", () => {
    ...
    */
    ...
    const result1 = findSuccessor(root, root.left.right); // Node 4's successor is 5
    expect(result1).toBe(root);

    const result2 = findSuccessor(root, root.left.left); // Node 2's successor is 3
    expect(result2).toBe(root.left);

    const result3 = findSuccessor(root, root); // Root node's successor is 8
    expect(result3).toBe(root.right);

    const result4 = findSuccessor(root, root.right.right); // Node 9 has no successor
  ...
  it("should return null for a node with no successor", () => {
    ...
    */
    ...
    const result = findSuccessor(root, root.right); // Node 2 has no successor
